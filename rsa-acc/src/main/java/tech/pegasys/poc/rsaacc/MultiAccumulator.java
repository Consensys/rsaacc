...
//    num = num.subtract(BigInteger.TEN);
    ...
    RsaAcc2 mainAccumulator = new RsaAcc2();
    RsaAcc2[] witnessAccumulators = new RsaAcc2[NUM_PRIMES];
    for (int i = 0; i < NUM_PRIMES; i++) {
      witnessAccumulators[i] = new RsaAcc2();
    }
...
//      BigInteger t4 = t3.mod(RsaAcc2.MODULUS);
      System.out.println("Checking Bezout Coefficients: check value (a*x + b*y mod N): " + t3);


      // Calculate w1**b * w2**a
      BigInteger temp1 = accOfPrimeToRemove.modPow(bezoutCoefficients[1], RsaAcc2.MODULUS);
      BigInteger temp2 = witnessAccumulators[i].getAccumulator().modPow(bezoutCoefficients[0], RsaAcc2.MODULUS);
    ...
    BigInteger modInv = five.modInverse(phiN);
  ...
  public static void proveMembership(RsaAcc2 mainAcc, RsaAcc2[] witnessAccs, int indexOfPrime) {
